...
## 시간 복잡도
처음에 2부터 $$n$$까지의 숫자를 모두 표시하고 2부터 시작하여 각 숫자의 배수들을 지우는 과정에서,
- $$p = 2$$에서 $$2^2, 2^3, 2^4, \dots$$가 지워지고 약 $$\frac{n}{2}$$번의 연산이 진행된다.
- $$p = 3$$에서 $$3^2, 3^3, 3^4, \dots$$가 지워지고 약 $$\frac{n}{3}$$번의 연산이 진행된다.
- 모든 $$p$$에 대하여 배수들을 지우는 횟수는 약 $$\frac{n}{p}$$번이다.

전체 연산 횟수는 각 $$p$$에 대하여 $$\frac{n}{p}$$번씩 연산이 일어나는데, 이 과정에서 $$p$$는 소수일 때만 연산이 이루어지므로 소수인 $$p$$의 $$n$$ 이하의 최소가 $$p_l$$이라 하면, 총 연산 횟수는 $$\sum_{p는 소수} \frac{n}{p}$$ 이고 이 과정에서 $$p$$는 소수일 때만 이루어진다. 이 합은 근사적으로 $$\sum_{p는 소수} \frac{1}{p}$$이고 이 합은 **소수의 조화급수**&ZeroWidthSpace;라고 알려져 있다.

오일러에 의하여 소수의 조화급수(*Prime Harmonic Series*)는 발산함이 알려졌는데, 그 증가치는 대략적으로 $$\sum_{p는 소수, p≤n} \frac{1}{p} ≥ \log{\log{n+1}} － \log{\frac{\pi^2}{6}}$$ 정도로 나타남이 알려져 있다.

![](https://velog.velcdn.com/images/aoi-aoba/post/3abc3aac-1d31-46ba-99a3-90f8ad71c7d0/image.png)

자세한 내용은 아래 위키피디아 페이지 참고.

https://en.wikipedia.org/wiki/Divergence_of_the_sum_of_the_reciprocals_of_the_primes

따라서, 에라토스테네스의 체 알고리즘의 시간 복잡도는 $$O(N \log \log N)$$에 비례하며, 이 역시 발산하긴 하나 그 발산의 속도와 정도가 매우 느리기 때문에 일반적으로 효율적이라 불리는 $$O(N \log{N})$$보다도 효율적이다.
